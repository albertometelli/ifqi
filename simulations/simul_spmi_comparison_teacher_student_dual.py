from spmi.envs.teacher_student_dual import TeacherStudentEnv
...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_literals",required=True, type=int)
    parser.add_argument("--max_value",required=True, type=int)
    parser.add_argument("--max_update", required=True, type=int)
    parser.add_argument("--max_literals_in_examples", required=True, type=int)
    parser.add_argument("--horizon", required=False,type=int,  default=10)
    args,_ = parser.parse_known_args([arg for arg in sys.argv[1:] if arg not in ('-h', '--help')])
    startTime = time.time()

    mdp = TeacherStudentEnv(n_literals=args.n_literals,
                        max_value=args.max_value,
                        max_update=args.max_update,
                        max_literals_in_examples=args.max_literals_in_examples,
    ...
    path_name = 'data/' + simulation_name
    ...
    uniform_policy = mdp.get_uniform_policy()
    ...
    policy_chooser = DoNotCreateTransitionsGreedyPolicyChooser(
        uniform_policy.get_rep(), mdp.nS, mdp.nA)
    model_chooser = DoNotCreateTransitionsGreedyModelChooser(mdp.P, mdp.nS,
                                                             mdp.nA)
