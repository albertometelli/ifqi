...
class SPMI(object):
    ...
    def __init__(self, mdp, eps, max_iter=30000, delta_q=None):
        """
    ...
    def safe_policy_model_iteration(self, initial_policy, initial_model):
        ...
        while ((p_er_adv + m_er_adv) > convergence) and self.iteration < iteration_horizon:
            ...
            if not self.policy_equiv_check(target, target_old) and not self.policy_equiv_check(policy, target_old):
                ...
                p_bound_old = ((er_adv_old ** 2) * (1 - gamma)) / (2 * self.delta_q * gamma * dist_sup_old * dist_mean_old)
                p_bound = ((p_er_adv ** 2) * (1 - gamma)) / (2 * self.delta_q * gamma * p_dist_sup * p_dist_mean)
            ...
            if p_dist_sup != 0 and p_dist_mean != 0:
                alfa_star = ((1 - gamma) * p_er_adv) / (self.delta_q * gamma * p_dist_sup * p_dist_mean)
                alfa = min(1, alfa_star)

            # beta star
            beta = 0
            if m_dist_sup != 0 and m_dist_mean != 0:
                beta_star = ((1 - gamma) * m_er_adv) / (self.delta_q * (gamma ** 2) * m_dist_sup * m_dist_mean)
            ...
            for alfa, beta in [(0, beta), (alfa, 0), (alfa, 1), (1, beta)]:
                bound = alfa * p_er_adv + beta * m_er_adv - (gamma / (self.delta_q * (1 - gamma)) *\
    ...
    def spmi_sup_bound(self, initial_policy, initial_model):
        ...
        while (p_er_adv > convergence or m_er_adv > convergence) and self.iteration < iteration_horizon:
            ...
            if not self.policy_equiv_check(target, target_old) and not self.policy_equiv_check(policy, target_old):
                er_adv_old, dist_sup_old, dist_mean_old = self.policy_chooser_old(policy, target_old, d_mu, Q)
                p_bound_old = ((er_adv_old ** 2) * (1 - gamma)) / (2 * self.delta_q * gamma * dist_sup_old * dist_sup_old)
                p_bound = ((p_er_adv ** 2) * (1 - gamma)) / (2 * self.delta_q * gamma * p_dist_sup * p_dist_sup)
                # if the target_old is selected update the measures consistently
                if p_bound_old > p_bound:
                    p_er_adv = er_adv_old
                    p_dist_sup = dist_sup_old
            ...
            alfa = 0
            if p_dist_sup != 0:
                alfa_star = ((1 - gamma) * p_er_adv) / (self.delta_q * gamma * p_dist_sup * p_dist_sup)
                alfa = min(1, alfa_star)

            # beta star
            beta = 0
            if m_dist_sup != 0:
                beta_star = ((1 - gamma) * m_er_adv) / (self.delta_q * (gamma ** 2) * m_dist_sup * m_dist_sup)
                beta = min(1, beta_star)
