...
for i in range(online_iterations):
    ...
    dataset = collect_episodes(mdp, behavioral_policy, n_episodes=batch_size)
    ...
    optimal_parameter, N_star, history_offline_reinforce = offline_reinforce_cheb.optimize(
    ...
    if adaptive_batch_size:
        batch_size = max(min(N_star,N_max),N_min)
...
'''
fig,ax = plt.subplots()
ax.plot(range(len(batch_size_history)),batch_size_history)
ax.set_xlabel('Epoch')
